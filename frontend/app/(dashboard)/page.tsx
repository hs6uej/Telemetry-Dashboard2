...
const STATUS_COUNTS = (stations: Station[]) => ({
...
export default function DashboardPage() {
  ...
  const counts = STATUS_COUNTS(stations)
        ...
        <div className="hidden md:flex items-center gap-4 text-xs">
          {[
            ...
            <div key={key} className={cn('flex items-center gap-1', color)}>
              {icon}<span className="font-bold">{counts[key as keyof typeof counts]}</span>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-72 flex-none border-r border-border flex flex-col overflow-hidden">
          <Tabs value={tab} onValueChange={setTab} className="flex flex-col h-full">
            ...
            <TabsContent value="map" className="flex-1 overflow-y-auto m-0 mt-0">
              <div className="p-2">
                <p className="text-xs text-muted-foreground px-1 py-2">
                  สถานีทั้งหมด {stations.length} แห่ง — คลิกบนแผนที่เพื่อดูรายละเอียด
                </p>
                <StationTable stations={stations} selected={selected} onSelect={setSelected} />
              </div>
            </TabsContent>

            <TabsContent value="table" className="flex-1 overflow-y-auto m-0 mt-0">
              <div className="p-2">
                <StationTable stations={stations} selected={selected} onSelect={setSelected} />
              </div>
            </TabsContent>
